...
func init() {
	...
	tIndexBytes.key = []byte{1, 0, 255, 95, 0, 2, 99, 97, 116, 0, 2, 49, 69, 115, 74, 52, 79, 119, 79, 65, 100, 121, 119, 103, 56, 105, 77, 51, 100, 110, 72, 50, 79, 68, 72, 102, 106, 113, 0}
	tIndexBytes.start = []byte{1, 0, 255, 95, 0, 0}
	tIndexBytes.end = []byte{1, 0, 255, 95, 0, 255}

	tIndexCounterBytes.key = []byte{1, 0, 255, 95, 0, 2, 99, 97, 116, 0}
	tIndexCounterBytes.start = []byte{1, 0, 255, 95, 0, 0}
	tIndexCounterBytes.end = []byte{1, 0, 255, 95, 0, 255}
}
...
func TestIRIIndexCompose(t *testing.T) {
	...
	// std key
	x.bytes([]byte(i1.GetKey(testDb.DbIndex)), tIndexBytes.getKey(testDb.DbIndexBytes), "GetKey()")

	rStart, rEnd := i1.GetClearRange(testDb.DbIndex).FDBRangeKeys()
	x.bytes([]byte(rStart.FDBKey()), tIndexBytes.getStart(testDb.DbIndexBytes), "GetClearRange(start)")
	x.bytes([]byte(rEnd.FDBKey()), tIndexBytes.GetEnd(testDb.DbIndexBytes), "GetClearRange(end)")

	// counter key
	x.bytes([]byte(i1.GetCounterKey(testDb.DbIndexCnt)), tIndexCounterBytes.getKey(testDb.DbIndexCntBytes), "GetKey()")

	crStart, crEnd := i1.GetCounterClearRange(testDb.DbIndexCnt).FDBRangeKeys()
	x.bytes([]byte(crStart.FDBKey()), tIndexCounterBytes.getStart(testDb.DbIndexCntBytes), "GetClearRange(start)")
	x.bytes([]byte(crEnd.FDBKey()), tIndexCounterBytes.GetEnd(testDb.DbIndexCntBytes), "GetClearRange(end)")
}
...
func TestIRIIndexParse(t *testing.T) {
	...
	// std key
	x.bytes([]byte(i2.GetKey(testDb.DbIndex)), tIndexBytes.getKey(testDb.DbIndexBytes), "GetKey()")
	...
	// counter key
	x.bytes([]byte(i2.GetCounterKey(testDb.DbIndexCnt)), tIndexCounterBytes.getKey(testDb.DbIndexCntBytes), "GetKey()")
...
func TestIRIIndexBadSignature(t *testing.T) {
	...
	// nothing
	x.error((&iri.NodeIndex{}).ValidateIRI(p), "null record on index.IndexId")
