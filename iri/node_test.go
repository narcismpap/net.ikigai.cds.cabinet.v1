...
func init() {
	...
	tNodeBytes.key = []byte{1, 0, 255, 128, 0, 2, 49, 69, 115, 74, 52, 79, 119, 79, 65, 100, 121, 119, 103, 56, 105, 77, 51, 100, 110, 72, 50, 79, 68, 72, 102, 106, 113, 0}
	tNodeBytes.start = []byte{1, 0, 255, 128, 0, 2, 49, 69, 115, 74, 52, 79, 119, 79, 65, 100, 121, 119, 103, 56, 105, 77, 51, 100, 110, 72, 50, 79, 68, 72, 102, 106, 113, 0, 0}
	tNodeBytes.end = []byte{1, 0, 255, 128, 0, 2, 49, 69, 115, 74, 52, 79, 119, 79, 65, 100, 121, 119, 103, 56, 105, 77, 51, 100, 110, 72, 50, 79, 68, 72, 102, 106, 113, 0, 255}
}
...
func TestIRINodeCompose(t *testing.T) {
	...
	x.bytes([]byte(n1.GetKey(testDb.DbNode)), tNodeBytes.getKey(testDb.DbNodeBytes), "GetKey()")

	rStart, rEnd := n1.GetClearRange(testDb.DbNode).FDBRangeKeys()
	x.bytes([]byte(rStart.FDBKey()), tNodeBytes.getStart(testDb.DbNodeBytes), "GetClearRange(start)")
	x.bytes([]byte(rEnd.FDBKey()), tNodeBytes.GetEnd(testDb.DbNodeBytes), "GetClearRange(end)")
}
...
func TestIRINodeParse(t *testing.T) {
	...
	x.bytes([]byte(n2.GetKey(testDb.DbNode)), tNodeBytes.getKey(testDb.DbNodeBytes), "GetKey()")
...
func TestIRINodeBadSignature(t *testing.T) {
	...
	// empty
	x.error((&iri.Node{}).ValidateIRI(p), "null record on node.type")
