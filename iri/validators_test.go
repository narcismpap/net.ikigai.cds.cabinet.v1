...
func TestValidateNodeId(t *testing.T) {
	...
	for v1 := range valid{
		...
		r.IsNoError(err1, fmt.Sprintf("ValidateNodeId(%s)", valid[v1]))
	}

	for v2 := range invalid{
		_, err2 := iri.ValidateNodeId(invalid[v2])
		r.IsError(err2, fmt.Sprintf("ValidateNodeId(%s)", invalid[v2]))
	}
}
...
func TestValidateUuid(t *testing.T) {
	...
	for v1 := range valid{
		...
		r.IsNoError(err1, fmt.Sprintf("ValidateUuid(%s)", valid[v1]))
	}

	for v2 := range invalid{
		_, err2 := iri.ValidateUuid(invalid[v2])
		r.IsError(err2, fmt.Sprintf("ValidateUuid(%s)", invalid[v2]))
	}
}
...
func TestValidateSequence(t *testing.T) {
	...
	for _, v1 := range []uint16{10, 12, 65000, 1000}{
		r.IsTrue(iri.ValidateSequence(v1), fmt.Sprintf("ValidateSequence(%d)", v1))
	}

	for _, v2 := range []uint16{0}{
		r.IsFalse(iri.ValidateSequence(v2), fmt.Sprintf("ValidateSequence(%d)", v2))
	}
}
